#!/usr/bin/env python3
"""
Resume 50-Episode Experiment from Algorithm 3
Continues from where the previous experiment left off
"""
...
class ResumeFiftyEpisodeExperiment:
    def __init__(self, episodes=50, existing_results_dir=None, output_dir='./experiments/results'):
        ...
        if existing_results_dir:
            self.output_dir = existing_results_dir
        else:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            self.output_dir = os.path.join(output_dir, f"experiment_50ep_5alg_resumed_{timestamp}")
            os.makedirs(self.output_dir, exist_ok=True)
        
        self.start_time = time.time()
        self.log_file = os.path.join(self.output_dir, 'experiment_resumed.log')
        
        # Load existing results after log_file is set
        if existing_results_dir:
            self.load_existing_results()
        else:
            self.results = {}
        
        self.log(f"50-Episode 5-Algorithm Experiment RESUMED")
    ...
    def load_existing_results(self):
        ...
        if os.path.exists(results_path):
            ...
            self.log(f"Loaded existing results from {results_path}")
            
            # Log what we have
            for game in self.results:
                if self.results[game]:
                    self.log(f"  {game}: {list(self.results[game].keys())}")
        else:
            self.results = {}
            self.log("No existing results found, starting fresh")
    ...
    def run_remaining_experiments(self):
        """Run only the remaining algorithms that haven't been completed"""
        ...
        self.log(f"\nResuming experiments on {', '.join(self.game_names)}")
        ...
        for env_name, game_name in zip(self.games, self.game_names):
            ...
            # Initialize results for this game if not present
            if game_name not in self.results:
                self.results[game_name] = {}
            
            # Get the priority time if it exists
            priority_time = None
            if 'DQN + Priority' in self.results[game_name]:
                priority_time = self.results[game_name]['DQN + Priority'].get('elapsed', 600)
                self.log(f"Using existing baseline time: {priority_time:.1f}s")
            
            for i, (alg_name, alg_type, config_fn) in enumerate(algorithms):
                # Skip if already completed
                if alg_name in self.results[game_name]:
                    self.log(f"\nAlgorithm {i+1}/5: {alg_name} - ALREADY COMPLETED, skipping")
                    continue
    ...
    def create_results_md(self):
        ...
        with open(md_path, 'w') as f:
            f.write(f"# 50-Episode 5-Algorithm RL Comparison (RESUMED)\n\n")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Resume 50-Episode 5-Algorithm Experiment')
    parser.add_argument('--episodes', type=int, default=50, help='Episodes per algorithm')
    parser.add_argument('--existing-dir', type=str, 
                       default='./experiments/results/experiment_50ep_5alg_20250806_221205',
                       help='Directory with existing results to resume from')
    args = parser.parse_args()
    
    experiment = ResumeFiftyEpisodeExperiment(
        episodes=args.episodes,
        existing_results_dir=args.existing_dir
    )
    experiment.run_remaining_experiments()
