...
class MiniGridPPOMHNetwork(nn.Module):
    ...
    def forward(self, state, c = None):
        ...
        return f
#####################################################################################
class MiniGridPPOMixtureMHNetworkGRIN(nn.Module):
    def __init__(self, input_shape, 
                       output_shape, 
                       n_features,
                       n_contexts = 1,
        ...
        self.num_grin_recurrence = 1 

        self.get_activation_list = ['_task_encoder']
        #self.get_activation_list += [
        #    f'cnn.1.model_layers.{str(i)}.5' for i in range(n_experts)
        #]
        
        self._h_activations = dict()
        self._hooks = []
        self.grin_inhibition_network = nn.Sequential(
            nn.Linear(n_experts + n_flatten + self._n_output, 2 * n_features[0]),
            nn.Tanh(),
            nn.Linear(n_features[0] * 2, n_experts + n_flatten),
            nn.Sigmoid(),
        )
    ...
    def forward(self, state, c = None):
        ...
        for i in range(self.num_grin_recurrence):
            q = self.forward_once(state, c, recurrent_pass=True)
        return q 
    ...
    def forward_once(self, state, c = None, recurrent_pass = False):
        ...
        if recurrent_pass is True: 
            ...
            pretex_features = torch.cat((pretex_features, self._h_activations['features_cnn']), axis = 1)    
            pretex_features = torch.cat((pretex_features, self._h_activations['f']), axis = 1)
        ...
        if recurrent_pass is True:
            features_cnn = features_cnn * features_inhibition_logits

        # only activation after weighting the features 
        features_cnn = torch.tanh(features_cnn)
        
        self._h_activations['features_cnn'] = features_cnn
