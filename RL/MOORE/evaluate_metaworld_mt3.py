#!/usr/bin/env python3
"""
Evaluation script for trained MetaWorld MT3 models with MOORE
"""
...
def evaluate_model(checkpoint_dir, n_episodes=10, render=False, verbose=True):
    ...
    Args:
        checkpoint_dir: Path to the directory containing the saved model
        n_episodes: Number of episodes to evaluate per task
    ...
    if not os.path.exists(agent_path):
        print(f"Error: Agent checkpoint not found at {agent_path}")
        ...
        if os.path.exists(agent_dir):
            ...
            if available_agents:
                print(f"Available checkpoints: {available_agents}")
                ...
                raise FileNotFoundError(f"No agent checkpoints found in {agent_dir}")
        else:
