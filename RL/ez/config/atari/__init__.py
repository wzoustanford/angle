...
class AtariConfig(BaseConfig):
    ...
    def new_game(self, seed=None, save_video=False, save_path=None, video_callable=None, uid=None, test=False, final_test=False):
        ...
        if save_video:
            ...
            # Create a video name based on uid if provided
            name_prefix = f"video_{uid}" if uid is not None else "video"
            env = RecordVideo(env, video_folder=save_path, name_prefix=name_prefix, 
                            episode_trigger=video_callable if video_callable else lambda episode: True)
        return AtariWrapper(env, discount=self.discount, cvt_string=self.cvt_string, seed=seed)
